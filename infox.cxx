...
int main()
{
	...
	do
	{
		...
		cin >> menu;
		if(menu<1 || menu>4)cout<<"\nDeve inserire un numero compreso tra 1 e 4\n"<<endl;
        }while(menu<1 || menu>4);
...
for (int a = 0; a < contatore; a++)
			{
				...
				cout << "Nome macchina: "<<cars[a].nome
					 << endl;
			if (cars[a].presa == true)
			{
				prenotata = "Prenotata";
				cout << prenotata << endl;
				for(int pren =0; pren < cars[a].n_pren; pren++ )
                    {
				cout << "Cliente che ha prenotato la macchina: " << cars[a].cliente_nome << " " << cars[a].cliente_cognome << endl
					 << "Prenotata da " << cars[a].inizio[pren].giorno << "/" << cars[a].inizio[pren].mese << "/" << cars[a].inizio[pren].anno << " a " << cars[a].fine[pren].giorno << "/" << cars[a].fine[pren].mese << "/" << cars[a].fine[pren].anno << endl
					 << endl;
				}
			}
...
break;
	        	...
	        	case 1:
    {
            ...
            for(int a=0; a < contatore; a++)
             ...
             /*             for(pren =0; pren < cars[a].n_pren ; pren++)
               ...
               && giornof < cars[a].inizio[pren].giorno|| annoi == cars[a].fine[pren].anno && mesei == cars[a].fine[pren].mese && giornoi>cars[a].fine[pren].giorno)
                  ...
                  //  if(controllo10==cars[a].n_pren)verifica=true;
               }

	        		if(controllo10==cars[a].n_pren || verifica1== true)
                    {

	        		if(contafrase=0)cout << "\nEcco l'elenco delle auto presenti nel sistema che non sono prenotate.\n" 	 << endl;
                        cout << "Numero macchina " << a << endl;
		...
		cout << "Vuole noleggiare quest'auto? \nDigitare 1 se vuole proseguire, 2 se vuole tornare al menu' princiale." << endl;
		cin >> conferma;
		...
		if (conferma == 1)
		{
...

			{
			    if(contatore1==0)
                {
                    cout<<"\nNon abbiamo clienti registrati nel nostro database,\nverra' reinderizzato verso la schermata di registrazione\n"<<endl;
                registrazione=2;
                }
